...
void read(std::string fn, vec_t &data) {
    ...
    std::cout << format_str("load: %s", fn.c_str()) << std::endl;
}
...
void mfcc(const std::string &fn, vec_t &ret) {
    vec_t raw;
    ...
    // MFCCの値を1/100にする（アドホック）
    std::for_each(mfcc.begin(), mfcc.end(), [](float &f) { f /= 100.0f; });

    ret.resize(MFCC_DIM);
    std::copy(mfcc.begin(), mfcc.end(), ret.begin());
}
} // namespace wav
