...
void fit_1vs1(int                                   n_class,
    ...
    for (int l = 0; l < n_class; l++) {
        for (int r = l + 1; r < n_class; r++) {
            ...
            classifier::binary::svm cls = classifier::binary::svm();
            cls.fit(data, label);
...
int predict_1vs1(int                                   n_class,
                 std::vector<classifier::binary::svm> &classifiers,
                 vec_t                                &v) {
    // predictの結果を加算して多数決で予測する
    std::vector<int> vote(n_class);
    int              i = 0;
    for (int l = 0; l < n_class; l++) {
        for (int r = l + 1; r < n_class; r++) {
            (classifiers[i++].predict(v) > 0) ? vote[l]++ : vote[r]++;
        }
    }

    // 同率一位ならunknownとして-1を返す
    int max   = *max_element(vote.begin(), vote.end());
    int count = 0;
    for (int i = 0; i < n_class; i++) {
        if (vote[i] == max) count++;
    }
    return (count == 1) ? max_index(vote) : -1;
}

void predict_1against1() {
}
} // namespace multiclass
} // namespace classifier
